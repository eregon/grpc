...
unless windows
  ...
  make = bsd ? 'gmake' : 'make'
  libgrpc_so = "#{grpc_lib_dir}/libgrpc.#{RbConfig::CONFIG['SOEXT']}.13.0.0"
  system("#{make} -j#{nproc} -C #{grpc_root} #{libgrpc_so} CONFIG=#{grpc_config} Q=")
...
$LDFLAGS << " -L#{grpc_lib_dir} -rpath #{grpc_lib_dir} -lgrpc"
